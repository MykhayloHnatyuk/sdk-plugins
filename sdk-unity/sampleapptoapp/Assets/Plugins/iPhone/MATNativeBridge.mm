...
extern "C" {
    ...
    void setDebugMode(bool isDebugMode)
    {
        ...
        [[MobileAppTracker sharedManager] setShouldDebugResponseFromServer:isDebugMode];
    }
    
    void setDeviceId(const char* device_id)
    {
        NSLog(@"Native: setDeviceId = %@", CreateNSString(device_id));
        
        [[MobileAppTracker sharedManager] setDeviceId:CreateNSString(device_id)];
    }
    ...
    void trackAction(const char* action, int isId, double revenue, const char*  currency)
    {
    ...
    void trackActionWithEventItem(const char* action, bool isId, MATEventItem eventItems[], int eventItemCount, const char* refId, double revenue, const char* currency, int transactionState)
    {
        ...
        NSLog(@"eventName       : %@", CreateNSString(action));
        NSLog(@"refId           : %@", CreateNSString(refId));
        NSLog(@"revenueAmount   : %f", revenue);
        NSLog(@"transactionState: %i", (unsigned int)transactionState);
        NSLog(@"currencyCode    : %@", CreateNSString(currency));
        
        // print the items array
        NSLog(@"MATSDK events array: %i", eventItemCount);
        for (uint i = 0; i < eventItemCount; i++) {

            MATEventItem eventItem = eventItems[i];
            
            NSLog(@"Item %@, unitPrice %f, quanity %i, revenue %f",
                  CreateNSString(eventItem.item),
                  eventItem.unitPrice,
                  eventItem.quantity,
                  eventItem.revenue);
        }
